...
{png(paste0(plotpath, 'access_forest_exclude.png'),
    ...
    height = 2500,
    pointsize = 40)
par(mar=c(0.1,0.1,0.1,3.1), oma=c(4.1,0.1,0.1,0.1), mfrow=c(2,1))
plot(nw_idn_ras, col = "grey80", legend = F, 
     xlab = "", ylab = "", xaxt="n", yaxt="n",
     #main = "Boolean constraint: 20% Least Accessible Sites Excluded",
     cex.main=1.4, cex.lab=1.3, legend.mar=3.5)
plot(tmpaccess, col = viridis(100), add = TRUE, 
     legend.args=list(text="Urban accessibility", font=2, side=2, line=1, cex=1.2), 
     legend.mar=3)
...
plot(nw_idn_ras, col = "grey80", legend = F,
     ...
     cex.main=1.4, cex.lab=1.3, legend.mar=3.5)
plot(tmpforest, col = viridis(100), add = TRUE, 
     legend.args=list(text="Forest cover", font=2, side=2, line=1, cex=1.2), 
     legend.mar=3.5)
...
par(xpd=TRUE)
legend(0.9,-0.95, "Excluded sites", fill="grey80", cex=1.2)
dev.off()}
